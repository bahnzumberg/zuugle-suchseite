 ...
 * 0001_baseline
 *
 * Baseline schema for the zuugle search database. This is a faithful port of the
 * former hand-maintained `database.sql`, which this migration replaces as the
 * single source of truth for the schema.
 *
 * It intentionally embeds the exact proven DDL via knex.raw (rather than
 * reconstructing it with knex.schema.createTable) so that a fresh `migrate:latest`
 * produces a schema byte-for-byte equivalent to the old init script — the exotic
 * types (tsvector, vector(1024), decimal precision, time, composite SERIAL keys)
 * and index definitions (HNSW, GIN, GiST/earthdistance, trigram, covering) must
 * match exactly. Verify with `pg_dump --schema-only`.
 *
 * Differences from the old database.sql, by design:
 *   - The leading `DROP TABLE` block is omitted (up() runs on a fresh schema).
 *   - The `kpi` INSERT … SELECT rows are omitted — that data is derived and is
 *     recomputed by the daily load (writeKPIs()).
 *
...
exports.down = async function (knex) {
    // Mirrors the old database.sql DROP block, plus the three tables it missed
    // (poi2tour, pois, search_suggestions) and the trigger/function.
    await knex.raw(`
DROP TRIGGER IF EXISTS trg_update_tour_image ON tour;
...
DROP FUNCTION IF EXISTS sync_tour_image_to_flat();
`);
};
