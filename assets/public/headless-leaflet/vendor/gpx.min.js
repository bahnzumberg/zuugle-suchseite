...
((L.GPX = L.FeatureGroup.extend({
    ...
    get_speed_data: function () {
        var e = this;
        return this._info.speed._points.map(function (t) {
            return e._prepare_data_point(t, e.m_to_km, e.ms_to_kmh, function (t, e) {
    ...
    get_speed_data_imp: function () {
        var e = this;
        return this._info.speed._points.map(function (t) {
            return e._prepare_data_point(t, e.m_to_mi, e.ms_to_mih, function (t, e) {
    ...
    _parse_gpx_data: function (t, e) {
        var n,
            i = [];
        (0 < (c = t.getElementsByTagName("name")).length && (this._info.name = c[0].textContent),
        ...
        if (-1 < o.indexOf("route"))
            for (var a = t.getElementsByTagName("rte"), _ = 0; _ < a.length; _++)
                i = i.concat(this._parse_segment(a[_], e, {}, "rtept"));
        if (-1 < o.indexOf("track")) {
            var r = t.getElementsByTagName("trk");
            for (_ = 0; _ < r.length; _++) {
                ...
                if (e.gpx_options.joinTrackSegments)
                    ...
                    for (var m = s.getElementsByTagName("trkseg"), p = 0; p < m.length; p++)
        ...
        if (
            ...
            for (n = t.getElementsByTagName("wpt"), _ = 0; _ < n.length; _++) {
                var h,
                    g = new L.LatLng(n[_].getAttribute("lat"), n[_].getAttribute("lon")),
                    u = n[_].getElementsByTagName("name"),
                    c = 0 < u.length ? u[0].textContent : "",
                    f = n[_].getElementsByTagName("desc"),
                    d = 0 < f.length ? f[0].textContent : "",
                    v = n[_].getElementsByTagName("sym"),
                    x = 0 < v.length ? v[0].textContent : null,
                    I = n[_].getElementsByTagName("type"),
                    y = 0 < I.length ? I[0].textContent : null,
                    u = e.marker_options.wptIcons,
                    f = e.marker_options.wptIconUrls,
                    v = e.marker_options.wptIconsType,
                    I = e.marker_options.wptIconTypeUrls,
                    k = e.marker_options.pointMatchers || [];
                if (u && x && u[x]) h = u[x];
                else if (v && y && v[y]) h = v[y];
                else if (f && x && f[x]) h = new L.GPXTrackIcon({ iconUrl: f[x] });
                else if (I && y && I[y]) h = new L.GPXTrackIcon({ iconUrl: I[y] });
                else if (0 < k.length) {
                ...
                h
                    ? ((g = new L.Marker(g, {
                          ...
                          .bindPopup("<b>" + c + "</b>" + (0 < d.length ? "<br>" + d : ""))
                          .openPopup(),
                      this.fire("addpoint", { point: g, point_type: "waypoint", element: n[_] }),
                      i.push(g))
                    : console.log(
    ...
    _parse_segment: function (t, e, n, i) {
        ...
        if (!o.length) return [];
        for (var a = [], _ = [], r = [], s = null, l = 0; l < o.length; l++) {
            var m = new L.LatLng(o[l].getAttribute("lat"), o[l].getAttribute("lon"));
            ((m.meta = { time: null, ele: null, hr: null, cad: null, atemp: null, speed: null }),
                0 < (p = o[l].getElementsByTagName("time")).length
                    ? (m.meta.time = new Date(Date.parse(p[0].textContent)))
                    : (m.meta.time = new Date("1970-01-01T00:00:00")));
            var p,
                h = null != s ? Math.abs(m.meta.time - s.meta.time) : 0;
            0 < (p = o[l].getElementsByTagName("ele")).length
                ? (m.meta.ele = parseFloat(p[0].textContent))
                : (m.meta.ele = null != s ? s.meta.ele : null);
            var g = null != s ? m.meta.ele - s.meta.ele : 0,
                u = null != s ? this._dist3d(s, m) : 0;
            if (
                (0 < (p = o[l].getElementsByTagName("speed")).length
                    ? (m.meta.speed = parseFloat(p[0].textContent))
                    : (m.meta.speed = 0 < h ? (1e3 * u) / h : 0),
                0 < (p = o[l].getElementsByTagName("name")).length)
            )
                for (
                    var c = p[0].textContent, f = e.marker_options.pointMatchers || [], d = 0;
                    d < f.length;
                    d++
                )
                    if (f[d].regex.test(c)) {
                        _.push({ label: c, coords: m, icon: f[d].icon, element: o[l] });
                        break;
                    }
            (0 < (p = o[l].getElementsByTagNameNS("*", "hr")).length &&
                ((m.meta.hr = parseInt(p[0].textContent)),
                this._info.hr._points.push([this._info.length, m.meta.hr]),
                (this._info.hr._total += m.meta.hr)),
                0 < (p = o[l].getElementsByTagNameNS("*", "cad")).length &&
                    ((m.meta.cad = parseInt(p[0].textContent)),
                    this._info.cad._points.push([this._info.length, m.meta.cad]),
                    (this._info.cad._total += m.meta.cad)),
                0 < (p = o[l].getElementsByTagNameNS("*", "atemp")).length &&
                    ((m.meta.atemp = parseInt(p[0].textContent)),
                    this._info.atemp._points.push([this._info.length, m.meta.atemp]),
                    (this._info.atemp._total += m.meta.atemp)),
                m.meta.ele > this._info.elevation.max && (this._info.elevation.max = m.meta.ele),
                m.meta.ele < this._info.elevation.min && (this._info.elevation.min = m.meta.ele),
                this._info.elevation._points.push([this._info.length, m.meta.ele]),
                m.meta.speed > this._info.speed.max && (this._info.speed.max = m.meta.speed),
                this._info.speed._points.push([this._info.length, m.meta.speed]),
                null == s &&
                    null == this._info.duration.start &&
                    (this._info.duration.start = m.meta.time),
                (this._info.duration.end = m.meta.time),
                (this._info.duration.total += h),
                h < e.max_point_interval && (this._info.duration.moving += h),
                (this._info.length += u),
                0 < g
                    ? (this._info.elevation.gain += g)
                    : (this._info.elevation.loss += Math.abs(g)),
                a.push((s = m)));
        }
        ...
        (this.fire("addline", { line: n, element: t }),
            ...
            (e.marker_options.startIcon || e.marker_options.startIconUrl) &&
                ...
                this.fire("addpoint", { point: v, point_type: "start", element: o[0] }),
                r.push(v)),
            (e.marker_options.endIcon || e.marker_options.endIconUrl) &&
                ((v = new L.Marker(a[a.length - 1], {
                    clickable: e.marker_options.clickable,
                    icon:
                        e.marker_options.endIcon ||
                        new L.GPXTrackIcon({ iconUrl: e.marker_options.endIconUrl }),
                })),
                this.fire("addpoint", { point: v, point_type: "end", element: o[o.length - 1] }),
                r.push(v)));
        for (l = 0; l < _.length; l++) {
            var v = new L.Marker(_[l].coords, {
                clickable: e.marker_options.clickable,
                title: _[l].label,
                icon: _[l].icon,
            });
            (this.fire("addpoint", { point: v, point_type: "label", element: _[l].element }),
                r.push(v));
        }
        return r;
    },
    _extract_styling: function (t, e, n) {
        ...
        return (
            0 < e.length &&
                ...
                0 < (t = e[0].getElementsByTagName("linejoin")).length &&
                    (i.lineJoin = t[0].textContent),
                0 < (t = e[0].getElementsByTagName("dasharray")).length &&
                    (i.dashArray = t[0].textContent),
                0 < (t = e[0].getElementsByTagName("dashoffset")).length &&
                    (i.dashOffset = t[0].textContent)),
