...
| `country`                        | `/search`      | **Legacy - to be removed.** Will only show tours from that country. Country filtering has moved to `filter.countries` in the filter dialog. This param is kept for backward compatibility. If both filter.countries and country param are passed, filter.countries takes priority.                                                                                                                                                                     | `Schweiz`, `Italia`, `Österreich`                                                                            |

<!-- TODO: combine range and provider parameters with range setting in filter.  -->
<!-- TODO: remove `country` query param once backward compatibility is no longer needed. Country filtering now uses `filter.countries`. -->
