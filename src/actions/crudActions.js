...
export function loadOneReturnAll(
  ...
  dispatch({ type: typeBefore });
  return axios
    .get(route + id, { params: { domain: window.location.host } })
    .then((res) => {
      dispatch({
        type: typeDone,
...
export function loadSuggestions(searchPhrase, city, language) {
  console.log("searchPhrase,city,lang", searchPhrase, city, language);
  return axios
    .get(
      `searchPhrases?search=${searchPhrase}&city=${city}&language=${language}`
    )
    .then((res) => {
      console.log("suggestion apiss run", res.data);
      return res.data?.items;
    })
    .catch((err) => console.error("err while getting the suggestion", err));
}
